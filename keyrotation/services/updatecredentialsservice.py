    ...
    def update_datasource(
        ...
        if "name" in gateway:
            ...
            encrypted_credentials_string = (
                asymmetric_encryptor_service.encode_credentials(serialized_credentials)
            )
        ...
        credentials_details = CredentialsDetails(
            cred_type, encrypted_data, encrypted_connection, privacy_level
        )

        # Converting CredentialDetails class object to json string
        credentials_details_req = CredentialsDetailsRequest(
            credentials_details.__dict__
        )

        return self.make_update_datasource_patch_request(
            credentials_details_req, gateway_id, datasource_id, access_token
        )

    def make_update_datasource_patch_request(
        self, credentials_details_req, gateway_id, datasource_id, access_token
    ):
        """Makes API call
        ...
        api_response = requests.patch(
            endpoint_url,
            data=json.dumps(credentials_details_req.__dict__),
