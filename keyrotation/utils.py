...
cred_types = {"KEY": "Key", "WINDOWS": "Windows", "OAUTH2": "OAuth2", "BASIC": "Basic"}
...
class Utils:
    def validate_config(app):
        ...
        if authenticate_mode == "":
            return (
                "Please specify one of the two authentication modes in config.py file"
            )
        if (
            authenticate_mode.lower() != "masteruser"
            and authenticate_mode.lower() != "serviceprincipal"
        ):
    ...
    def serialize_credentials(credentials_arr, cred_type):
        ...
        if cred_type == cred_types["KEY"]:
            ...
            serialized_credentials = (
                "{'credentialData':[{'name':'accessToken','value':'"
                + credentials_arr[0]
                + "'}]}"
            )
